...
class Cube2Equirec(object):
    ...
    def _ToEquirec(self, batch, mode):
        ...
        for ori in range(6):
            grid = self.grid[ori, :, :, :].unsqueeze(0)
            mask = (self.orientation_mask == ori).unsqueeze(0)

            if self.CUDA:
                masked_grid = Variable(grid * mask.float().expand(-1, -1, -1, 2)).cuda()
            else:
                masked_grid = Variable(grid * mask.float().expand(-1, -1, -1, 2))

            source_image = batch[ori].unsqueeze(0)

            sampled_image = torch.nn.functional.grid_sample(source_image, masked_grid, mode=mode)

            if self.CUDA:
                sampled_image_masked = sampled_image * Variable(mask.float().view(1, 1, self.output_h, self.output_w).expand(1, ch, -1, -1)).cuda()
            else:
                sampled_image_masked = sampled_image * Variable(mask.float().view(1, 1, self.output_h, self.output_w).expand(1, ch, -1, -1))

            output = output + sampled_image_masked
...
def plot_figure(cube, equi, equi_gt):
    ...
    ax1.imshow(cube[0, :, :, :])  # back
    ax2.imshow(cube[1, :, :, :])  # down
    ax3.imshow(cube[2, :, :, :])  # front
    ax4.imshow(cube[3, :, :, :])  # left
    ax5.imshow(cube[4, :, :, :])  # right
    ax6.imshow(cube[5, :, :, :])  # up
    ax7.imshow(equi)
    ax8.imshow(equi_gt)
